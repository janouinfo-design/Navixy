...
- [x] **DOCUMENTS V2 GÉNÉRIQUE** (2026-06, TERMINÉ ET TESTÉ — 203 pytest passed + E2E 9/9 iteration_29): (1) Collection Mongo `documents` tenantisée (backend/documents_v2.py, router /api/documents sous module vehicules): métadonnées structurées {category_id, title, document_number, issued_at, valid_from, expiry_date, source manual/import/scan/ai/legacy, ai_extraction prévu NON implémenté (chantier carte grise IA), uploaded_by}, fichiers disque /app/backend/uploads/{tenant}/docs/. (2) Références véhicule: tracker_id (relation technique) + navixy_vehicle_id (canonique), reconcile_status linked/to_reconcile — JAMAIS de faux véhicule; docs to_reconcile visibles dans /api/vehicles/integrity; PATCH réconcilie sans réimport. (3) Catégories: 6 système (carte_grise SANS échéance [CH], assurance_rc→assurance critique, leasing→leasing, controle SANS échéance [anti double-comptage avec module Contrôles], facture, autre→document générique) + custom par tenant (POST/DELETE ADMIN only, 409 si utilisée). (4) BRANCHEMENT MOTEUR (deadline_engine v1.1.0): DOCUMENT_V2 priorité max; assurance/leasing = expiry la plus LOINTAINE (renouvellement); type générique "document" émis par doc avec flag critique de catégorie; statut JAMAIS stocké, badge par document calculé backend (embedded deadline dans GET /documents). VALIDÉ E2E: doc Assurance RC ajouté → badges fiche+liste+Vue générale basculent, suppression → fallback legacy. (5) Migration one-shot idempotente au startup: 3 docs legacy migrés (source=legacy, affichés "importé"); tableau vehicle_admin.documents[] conservé mais plus alimenté/affiché. (6) Frontend: DocumentsV2Tab.jsx (formulaire métadonnées, édition inline, aperçu/miniatures conservés, toasts erreurs); ancien DocumentsTab supprimé de VehiclesTab; BUG FIXÉ: badges contrôles individuels (clé dlMap tid:controle:source_id). (7) Audit trail: DOC_UPLOADED/DOC_UPDATED/DOC_DELETED/DOC_DOWNLOADED (téléchargement seulement, pas inline)/DOC_CATEGORY_*. (8) Validation dates format + ordre valid_from<=expiry_date (400). NOTES: picker date natif = format locale navigateur (cohérent avec le reste de l'app); iframe PDF vide en Chromium headless (texte de repli présent, non bloquant)
- [x] **PHASE SÉCURISATION PRÉ-DOCUMENTS V2** (2026-06, TERMINÉE ET TESTÉE): (1) Fixtures auth réparées — tests/conftest.py (login super admin réel, session lru_cache), 6 suites corrigées + backend_test.py, AVANT 125 pass/38 fail/8 err → APRÈS 190 pass/0 fail/1 skip (skip volontaire: photo garage, véhicule de test disparu du parc réel; tests vivants figés sur parc de février convertis en invariants — parc réel réorganisé: 21 conducteurs, Audi supprimée, groupe renommé Skoda). PIÈGE ÉVITÉ: ne jamais muter les headers de la session partagée (Content-Type json cassait les uploads multipart d'autres suites). (2) MOTEUR D'ÉCHÉANCES UNIQUE backend/deadline_engine.py + GET /api/vehicles/deadlines (tenantisé): statuts EXPIRED/DUE_SOON(<30j)/VALID, severity, règle 2b, NULL≠0 (date absente = aucune échéance), items traçables {source, source_field, source_id}. Précédence dual-read: DOCUMENT_V2 > NAVIXY_GARAGE(liability_insurance_valid_till) > VEHICLE_LEGACY. Consommateurs migrés: OverviewTab (fetch deadlines, plus AUCUN calcul JS), VehiclesTab (Badge item-based, dlMap, refetch après save), PDF (import direct du moteur, _echeance supprimé). (3) BUG C1 CORRIGÉ + test de régression unitaire + live (écriture garage réelle restaurée en finally) + démontrable en démo. (4) IDENTITÉ CANONIQUE: décision = navixy_vehicle_id référence quand présent, tracker_id = relation technique, VIN = vérification pas clé; GET /api/vehicles/integrity (rapport lecture seule, no_auto_merge) détecte orphelins/plaques dupliquées/VIN conflicts/liens ambigus. (5) HOOK DOCUMENTS V2 prêt: compute_vehicle_deadlines(documents_v2=[{document_id, deadline_type, expiry_date}]) priorité max, testé — aucun consommateur ne changera. (6) Correctifs post-E2E (iteration_28, 8/9 puis fixes re-vérifiés): EditableSection.save try/catch + toast sonner (Toaster monté App.js), champs internes assurance toujours visibles avec libellé de précédence, demo_simulator vehicle/read|update (overrides mémoire, SIMULATION only). Fichiers: backend/deadline_engine.py, backend/server.py (endpoints deadlines+integrity, PDF), backend/tests/conftest.py, backend/tests/test_deadline_engine.py (20 tests), backend/demo_simulator.py, frontend OverviewTab/VehiclesTab/App.js. RESTE À ARBITRER avant Documents V2: schéma référence documentaire {tenant, navixy_vehicle_id?, tracker_id?} (couvrir véhicules garage non liés), modèle métadonnées Document V2; maintenance/expertise sans colonne dans table Véhicules (préexistant, visible Vue générale)
- [x] **AUDIT CROISÉ Dashboard/Documents/Véhicules/Navixy** (2026-06, lecture seule, AUCUN code/donnée modifié): rapport complet /app/memory/AUDIT_CROISE_DOCUMENTS.md. Constats clés: (1) pas de module Documents autonome (onglet fiche véhicule) ; (2) 3 moteurs d'échéance dupliqués avec BUG SILENCIEUX C1: Overview ignore garage.liability_insurance_valid_till (assurance saisie garage invisible des alertes critiques) alors que fiche+PDF l'utilisent ; (3) pas de véhicule canonique (identité éclatée tracker/garage/caps/vehicle_admin, incohérences réelles Techlift: labels contradictoires + plaque VS 475 676 dupliquée) ; (4) carte grise IA/OCR = RIEN n'existe ; (5) conformité = libellé, pas de moteur ; (6) comptages réels: default 37 trk/27 garage liés/2 non liés/10 sans garage/1 fiche orpheline 781479, flex 52/38/0/14. Plan migration 6 étapes validé dans rapport (1: moteur échéances backend unique corrige C1 ; 2: Document V2 générique ; 3: dual-read dans le moteur seul ; 5: couverture mesurée ; 6: décommission progressif). BLOQUANTS (TOUS LEVÉS par la phase sécurisation): fixtures pytest auth, C1, identité canonique
